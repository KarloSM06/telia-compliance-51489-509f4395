      ...
      <div className="mx-auto max-w-[1440px] px-6 md:px-8 lg:px-16">
        <div className={cn("flex items-center justify-between transition-all duration-500", isScrolled ? "h-14" : "h-20")}>
          ...
          <div className="flex items-center">
            <button onClick={() => navigate("/")} className="flex items-center gap-4 group transition-all duration-500 hover:scale-[1.03]">
              <div className="relative">
                ...
                <img src={hiems_logo} alt="Hiems logo" className={cn("rounded-2xl shadow-card transition-all duration-500", "group-hover:shadow-glow group-hover:rotate-6", isScrolled ? "h-11 w-11" : "h-14 w-14")} />
              </div>
              <div className="flex flex-col items-start">
                <span className={cn("font-bold bg-gradient-primary bg-clip-text text-transparent", "transition-all duration-500 group-hover:tracking-wider", isScrolled ? "text-2xl" : "text-3xl")}>
                  Hiems
                </span>
                <span className={cn("hidden xl:block text-muted-foreground font-medium transition-all duration-500", isScrolled ? "text-[9px] -mt-0.5" : "text-[10px] -mt-1")}>Skräddarsydda AI-system</span>
              </div>
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            {/* Mobile Menu Toggle */}
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2.5 hover:bg-accent/10 rounded-xl transition-all duration-300 hover:scale-110 group" aria-label="Toggle menu">
              <div className="relative">
                {isMobileMenuOpen ? <X className="h-5 w-5 transition-transform duration-300 group-hover:rotate-90" /> : <Menu className="h-5 w-5 transition-transform duration-300 group-hover:scale-110" />}
              </div>
            </button>
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              {user ? <DropdownMenu>
                ...
                </DropdownMenu> : <Button variant="ghost" size="sm" onClick={handleAuthClick} className="hover:bg-accent/10 transition-all duration-300 hover:scale-105 font-medium">
                  Logga in
                </Button>}
              
              {/* CTA Button */}
              <Button className={cn("relative overflow-hidden bg-gradient-gold text-primary font-bold", "shadow-button hover:shadow-glow", "transition-all duration-500 hover:scale-105", "before:absolute before:inset-0 before:bg-white/30", "before:translate-x-[-100%] before:skew-x-12", "before:transition-transform before:duration-700", "hover:before:translate-x-[100%]", isScrolled ? "text-xs px-4 py-2 h-9" : "text-sm px-6 py-2.5 h-10")} onClick={() => setIsConsultationModalOpen(true)}>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-background/80 backdrop-blur-2xl border-b border-border/50 shadow-elegant animate-fade-in overflow-hidden">
            ...
            <div className="mx-auto max-w-[1440px] px-6 py-8 relative z-10">
              <div className="flex flex-col gap-6">
                ...
                {user ? <div className="space-y-4 pt-4 border-t border-border/30">
                  ...
                  </div> : <div className="pt-4 border-t border-border/30">
                    <Button variant="ghost" onClick={handleAuthClick} className="justify-start w-full hover:bg-accent/10 py-6 font-medium">
                      Logga in
                    </Button>
                  </div>}
