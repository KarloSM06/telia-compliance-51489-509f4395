...
export const ChartInsightsBox = ({ data, dataKey, type = 'general' }: ChartInsightsBoxProps) => {
  const insights = useMemo<Insight[]>(() => {
    ...
    if (Math.abs(change) > 10) {
      ...
      results.push({
        ...
        message: `${isPositive ? '↑' : '↓'} ${Math.abs(change).toFixed(1)}% ${
          isPositive ? 'ökning' : 'minskning'
        } från förra perioden ${isGood ? '🎉' : '⚠️'}`,
    ...
    if (lastValue > avg + 2 * stdDev || lastValue < avg - 2 * stdDev) {
      results.push({
        type: 'info',
        message: `Senaste värdet (${lastValue.toFixed(0)}) är ovanligt ${
          lastValue > avg ? 'högt' : 'lågt'
        } jämfört med genomsnittet (${avg.toFixed(0)})`,
        icon: AlertCircle,
      });
    }
    ...
    <div className="space-y-2 mb-3">
      {insights.map((insight, index) => {
        ...
        return (
          <div
            key={index}
            className={`flex items-start gap-2 p-2 rounded-lg border text-xs ${bgColor}`}
          >
            <Icon className={`h-4 w-4 mt-0.5 flex-shrink-0 ${textColor}`} />
            <p className={`${textColor} font-medium`}>{insight.message}</p>
          </div>
        );
      })}
    </div>
