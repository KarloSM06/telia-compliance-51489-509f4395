...
export const ScanningAnalysisVisual = () => {
  const checklist = [
    { label: "System check", status: "success" },
    { label: "Process check", status: "success" },
    { label: "Speed check", status: "warning" },
    { label: "Manual work", status: "error" },
    { label: "Repetitive task", status: "error" },
  ];

  return (
    <div className="relative h-[280px] w-full flex items-center justify-between gap-8 px-10 bg-white/60 backdrop-blur-md border border-gray-300 rounded-xl shadow-lg">
      ...
      <div className="relative w-[180px] h-[180px] flex-shrink-0">
        ...
        >
          <div
            className="absolute top-0 left-1/2 w-2 h-1/2 origin-bottom -translate-x-1/2"
            style={{
        ...
        <motion.div
          className="absolute top-1/2 left-1/2 w-6 h-6 bg-indigo-600 rounded-full -translate-x-1/2 -translate-y-1/2"
          animate={{ scale: [1, 1.4, 1], opacity: [1, 0.6, 1] }}
          transition={{ duration: 1.5, repeat: Infinity }}
        />
        <motion.div
          className="absolute top-1/2 left-1/2 w-4 h-4 bg-white/40 rounded-full -translate-x-1/2 -translate-y-1/2"
