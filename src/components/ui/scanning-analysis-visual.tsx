...
export const ScanningAnalysisVisual = () => {
  ...
  return <div className="relative h-full w-full flex items-center justify-between gap-2 md:gap-6 px-3 md:px-6 lg:px-10 bg-white/60 backdrop-blur-md border border-gray-300 rounded-xl shadow-lg">
      ...
      <div className="relative w-[100px] h-[100px] md:w-[140px] md:h-[140px] lg:w-[180px] lg:h-[180px] flex-shrink-0">
        ...
        <div className="absolute inset-0 animate-rotate" style={{
        ...
        {/* Center pulse - CSS animation */}
        <div className="absolute top-1/2 left-1/2 w-4 h-4 bg-white/40 rounded-full -translate-x-1/2 -translate-y-1/2 animate-pulse" />
      </div>

      {/* Right side: Workflow checklist - IMPROVED SPACING */}
      <div className="flex-1 space-y-1.5 md:space-y-3">
        {checklist.map((item, index) => <div key={item.label} className="flex items-center gap-2 md:gap-4 transition-transform duration-200 hover:scale-105 animate-fade-in-up" style={{
