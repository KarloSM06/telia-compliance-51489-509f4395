...
const BlogPostCard = React.forwardRef<HTMLDivElement, BlogPostCardProps>(
  ({ className, variant, tag, date, title, description, imageUrl, href, readMoreText = 'Läs mer' }, ref) => {
    ...
    return (
      <motion.div
        ...
        whileHover="hover"
      >
        <a href={href} className="absolute inset-0 z-10" aria-label={`Läs mer om ${title}`}>
          <span className="sr-only">Läs mer</span>
        </a>
