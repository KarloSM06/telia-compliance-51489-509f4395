...
export const EmailBriefingVisual: React.FC<EmailBriefingVisualProps> = ({
  className = ""
}) => {
  ...
  const stats = [{
    label: "Sent",
    value: animatedStats.sent.toLocaleString(),
    icon: Mail
  }, {
    label: "Opened",
    value: animatedStats.opened.toLocaleString(),
    icon: CheckCircle
  }, {
    label: "Clicked",
    value: animatedStats.clicked.toLocaleString(),
    icon: TrendingUp
  }, {
    label: "Replied",
    value: animatedStats.replied.toLocaleString(),
    icon: Mail
  }];
  return <div className={`h-auto min-h-[300px] md:h-[500px] w-full max-w-[900px] mx-auto bg-white/5 backdrop-blur-sm border-2 border-white/10 rounded-2xl shadow-lg overflow-hidden ${className}`}>
      {/* Header */}
      

      {/* Tab Navigation */}
      <div className="bg-white/5 backdrop-blur-sm border-b-2 border-white/10 px-4 md:px-8 py-2 flex gap-4 md:gap-6">
        {tabs.map(tab => <button key={tab} className={`text-sm font-medium pb-2 transition-all ${tab === activeTab ? 'text-gray-900 border-b-2 border-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
            {tab}
          </button>)}
      </div>
        ...
        delay: 0.2
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3 bg-white/5 backdrop-blur-sm border-2 border-white/10 rounded-lg p-3">
            ...
            return <motion.div key={stat.label} initial={{
              ...
              delay: 0.3 + idx * 0.1
                  ...
                  <motion.p className="text-base md:text-lg font-mono font-bold text-gray-900 mb-0.5" initial={{
                scale: 1
              }} animate={{
                scale: [1, 1.05, 1]
              }} transition={{
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative h-2 bg-gray-200 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-purple-600 rounded-full shadow-sm" initial={{
              scaleX: 0
            }} animate={{
              scaleX: 0.72
            }} transition={{
