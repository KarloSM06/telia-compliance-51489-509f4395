...
export const ExpertiseCategoryCard = ({
  ...
  return <Card className="group h-full overflow-hidden border-0 bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-all duration-500 hover:shadow-2xl">
          ...
          <CardContent className="flex-1 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-3">
                {category.items.map((item, idx) => <div key={idx} className="flex items-center gap-3 p-2 rounded-lg hover:bg-accent/50 transition-colors duration-200">
                    {item.logo ? <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-background rounded">
                        <img src={item.logo} alt={item.name} className="w-6 h-6 object-contain" />
                      </div> : <CheckCircle className="flex-shrink-0 h-5 w-5 text-primary" />}
